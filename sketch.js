...
function preload() 
{
  ...
  sensor = x; // !!!!sensor equals x bc arduino code
}
...
function mouseMoved() //What would I do for sensor activated??
{
	updateParams();
}
...
{
	if (mode === 0) {
		blackValue = int(map(sensor, 0, width - 1, -16777216, 0));
	} else if (mode == 1) {
		brightnessValue = int(map(sensor, 0, width - 1, 0, 255));
	} else if (mode == 2) {
		whiteValue = int(map(sensor, 0, width - 1, -16777216, 0));
	}
